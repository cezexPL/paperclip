  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">{t("bootstrap.instanceSetup")}</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {hasActiveInvite
            ? t("bootstrap.hasInvite")
            : t("bootstrap.noInvite")}
        </p>
  ...
  if (healthQuery.error) {
    return (
      <div className="mx-auto max-w-xl py-10 text-sm text-destructive">
        {healthQuery.error instanceof Error ? healthQuery.error.message : t("bootstrap.failedLoadState")}
      </div>
    );
  }
...
function OnboardingRoutePage() {
  ...
  const title = matchedCompany
    ? t("onboardingRoute.addAgentTo", { name: matchedCompany.name })
    : companies.length > 0
  ...
  const description = matchedCompany
    ...
    : companies.length > 0
      ...
      : t("onboardingRoute.getStartedDesc");
    ...
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        ...
        <div className="mt-4">
          ...
          >
            {matchedCompany ? t("agents.addAgent") : t("onboardingRoute.startOnboarding")}
          </Button>
        </div>
      </div>
    </div>
...
function NoCompaniesStartPage({ autoOpen = true }: { autoOpen?: boolean }) {
  ...
  return (
    <div className="mx-auto max-w-xl py-10">
      <div className="rounded-lg border border-border bg-card p-6">
        <h1 className="text-xl font-semibold">{t("onboardingRoute.createFirst")}</h1>
        <p className="mt-2 text-sm text-muted-foreground">
          {t("onboardingRoute.getStarted")}
        </p>
        <div className="mt-4">
          <Button onClick={() => openOnboarding()}>{t("onboardingRoute.newCompany")}</Button>
        </div>
      </div>
    </div>
  );
